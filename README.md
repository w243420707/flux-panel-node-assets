...
- 节点程序更新至 `3.1.7`，支持 VPS 累计流量与当前、前两台使用记录。
- 每 `5` 秒独立采样并原子保存累计状态，断开面板期间继续统计，重连补报；换 IP、普通重启和程序更新不清零。
- 上报用途限定的硬件／系统身份指纹、计量批次和序号，配合面板识别换机或重装；正常退出和重启前补写累计状态。
- 状态保存在 `/etc/gost/vps-usage.json`，更新脚本保留该文件；四种架构程序和校验文件与主仓库同步。
